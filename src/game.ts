...
export class Game{
  ...
  fire(uid:string,target:string){if(this.phase!=="playing")throw Error("The match has not started.");const a=this.current(),t=this.players.get(target);if(!a||a.uid!==uid||!t||!t.alive)throw Error("Invalid turn or target.");if(!this.shells.length)this.reload();const shell=this.shells.shift()!;if(shell==="live"){const dmg=a.sawed?2:1;a.sawed=false;t.charges-=dmg;if(t.charges<=0)t.alive=false;this.last=`${a.username} fired a LIVE shell.`;this.next()}else{this.last=`${a.username} fired a BLANK shell.`;if(target!==uid)this.next()}const alive=this.alive();if(alive.length<=1){this.phase="finished";this.last=`${alive[0]?.username??"Nobody"} wins the match.`}else if(!this.shells.length){this.round++;this.deal();this.reload()}}
