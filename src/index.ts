  ...
  async fetch(req:Request,env:Env){
    ...
    try{
      ...
      if(req.method==="POST"&&joinMatch){
        ...
        if(!data.ok || !data.room) return json({ok:false,message:data.message||"Room not found."},404,req,env);
      ...
      if(wsMatch){
        ...
        const data=await r.json() as any;
        if(!data.ok || !data.room) return new Response(JSON.stringify({ok:false,message:data.message||"Room not found."}),{status:404,headers:{"content-type":"application/json",...cors(origin,env.ALLOWED_ORIGIN)}});
        const room=data.room;
