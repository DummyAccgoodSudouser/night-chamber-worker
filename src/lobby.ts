    ...
    try{
      if(req.method==="POST"&&u.pathname==="/create"){
        ...
        for(let attempt=0;attempt<20;attempt++){
          ...
          const exists=this.ctx.storage.sql.exec("SELECT code FROM rooms WHERE code=?",candidate).toArray()[0];
      ...
      if(req.method==="GET"&&u.pathname.startsWith("/host/")){
        const code=u.pathname.slice(6).toUpperCase();
        const row=this.ctx.storage.sql.exec("SELECT host_uid as hostUid FROM rooms WHERE code=?",code).toArray()[0] as any;
        return row?Response.json({ok:true,hostUid:row.hostUid}):Response.json({ok:false,message:"Room not found."},{status:404});
      }
      if(req.method==="POST"&&u.pathname==="/adjust"){
        const b=await req.json() as {code?:string;delta?:number};
        const code=String(b.code||"").toUpperCase();
        const row=this.ctx.storage.sql.exec("SELECT players,max_players FROM rooms WHERE code=?",code).toArray()[0] as any;
      ...
      if(req.method==="POST"&&u.pathname==="/touch"){
        ...
        const room=this.ctx.storage.sql.exec("SELECT code FROM rooms WHERE code=?",String(b.code||"").toUpperCase()).toArray()[0];
