    ...
    try{
      ...
      if(req.method==="GET"&&u.pathname==="/public"){
        const rooms=this.ctx.storage.sql.exec(
          "SELECT code,id,mode,visibility,max_players as maxPlayers,players,status FROM rooms WHERE visibility='public' AND status='waiting' ORDER BY created_at DESC LIMIT 30"
        ).toArray();
